...
if(!empty($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") {
    ...
    if(isset($_SESSION["token"]) && $_SESSION["token"] === $formData["_token"])  //if tokens match
    {
        ...
        if(trim($formData["document"]) == "")
        {
            $errors[] = "Document field can't be blank.";
        }
         ...
         */
        $check_if_user_exists = $db->prepare("SELECT id FROM coordinator WHERE email = :email OR username = :username");
        ...
        if(empty($errors))
        {
            ...
            $create_user = $db->prepare("INSERT INTO coordinator(name, document, email, username, password) VALUES(:name, :document, :email, :username, :password)");
