$(document).ready(function() {
    ...
    $("#electionForm").submit(function(e) {
        e.preventDefault();
        $.post("ajax/election.php", { formData:$(this).serialize() }, function(data) {
            console.log(data);
            if(data.success === true) {
                $("#electionForm .alert-errors").html("").hide();
                $("#electionForm .alert-success").html("Se Han Registrado los Datos Con Exito").fadeIn(1000);
                setTimeout(function() {
                    window.location.reload();
                }, 2000);
            }
                ...
                errors_data += '</ul>';
                $("#electionForm .alert-errors").html(errors_data).hide().fadeIn(1000);
            }
        }, 'json');
    });
